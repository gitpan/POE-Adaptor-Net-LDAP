...
my $poe_states = {
	_start => sub {
		...
		$ldap_object->{_send_postback} = $session->postback( 'send_message' );
	...
	sf_success => sub {
		my ($heap, $sock, $addr, $port) = @_[HEAP, ARG0..ARG2];
...
=item POE::Adaptor::Net::LDAP->new( hostname, callback => $coderef, OPTIONS )
